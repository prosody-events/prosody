...
use crate::cassandra::{
    ...
    TABLE_NODE_DIRECTORY, TABLE_NODES_BY_GROUP, TABLE_SCHEMA_MIGRATIONS, TABLE_SEGMENTS,
    TABLE_SLABS, TABLE_TYPED_KEYS, TABLE_TYPED_SLABS,
};
...
/// from the `src/cassandra/migrations/` directory at compile time.
#[derive(RustEmbed)]
...
pub fn load_embedded_migrations(keyspace: &str) -> Result<Vec<Migration>, super::MigrationError> {
    ...
    for filename in MigrationAssets::iter() {
        ...
        content_str = content_str
            ...
            .replace("{{TABLE_KEYED_STATE_CELL}}", TABLE_KEYED_STATE_CELL)
            .replace("{{TABLE_KEYED_STATE_IDENTITY}}", TABLE_KEYED_STATE_IDENTITY)
            .replace(
                "{{TABLE_KEYED_STATE_PUBLICATION}}",
                TABLE_KEYED_STATE_PUBLICATION,
            )
            .replace("{{TABLE_NODE_DIRECTORY}}", TABLE_NODE_DIRECTORY)
            .replace("{{TABLE_NODES_BY_GROUP}}", TABLE_NODES_BY_GROUP);
