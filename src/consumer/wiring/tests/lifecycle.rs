...
use super::{
    Event, EventLog, RecordingBackend, RecordingDirectory, consumer_config, peer_config, start,
};
...
/// drops the client and therefore the handler provider, and only then lets the
/// coordinator deregister this node.
#[tokio::test(flavor = "multi_thread")]
async fn peer_teardown_follows_the_poll_loop() -> Result<()> {
    let log: EventLog = Arc::new(Mutex::new(Vec::new()));
    ...
    let consumer = start(&config, managers, heartbeats, Arc::clone(&log), attachment).await?;
    assert!(
        log.lock()
            .iter()
    ...
    let provider = events
        .iter()
        .position(|event| matches!(event, Event::ProviderDropped))
        .ok_or_else(|| eyre!("the poll loop did not drop its provider"))?;
    let deregistered = events
        .iter()
        .position(|event| matches!(event, Event::Deregistered))
        .ok_or_else(|| eyre!("peer teardown did not deregister the node"))?;
    ensure!(
        provider < deregistered,
        "peer teardown preceded the poll loop"
    );
    ensure!(
        events
            .iter()
