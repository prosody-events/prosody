...
impl PeerHandles {
    ...
    /// removed, and [`ShutdownError::Teardown`] when the coordinator ended
    /// without a report. Both leave a row that expires on its lease.
...
impl<D: NodeDirectory> PeerAttachment for PreparedPeer<D> {
    ...
    async fn activate(
        ...
        let runtime = match self.prepared.activate(group).await {
            Ok(runtime) => runtime,
            Err((prepared, error)) => {
                return Err((
                    Self { prepared },
...
async fn run_coordinator<D: NodeDirectory>(
    ...
    match stopped.await {
        ...
        Err(_) => {
            if let Err(error) = runtime.shutdown(|| async {}).await {
