...
fn requesting_excise(index: u8, request_byte: u8, key: &str) -> Result<ConsumerMessage<()>> {
    create_message_with_payload(
        key,
...
fn create_message(
    ...
    create_message_with_payload(key, request, span, Value::default())
}

fn create_message_with_payload<P: Default + Send + Sync + 'static>(
    key: &str,
    request: Option<ResultRequest>,
    span: Span,
    payload: P,
) -> Result<ConsumerMessage<P>> {
