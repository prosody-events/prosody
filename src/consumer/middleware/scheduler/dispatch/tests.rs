...
fn interleaved_bursts_both_classes() -> Result<()> {
    ...
    assert!(
        ...
        "Failure class should dominate phase 1: failure={phase1_failure} normal={phase1_normal}"
    );
...
fn class_proportion_recovery_from_skew() -> Result<()> {
    ...
    assert!(
        ...
        "Failure percentage should increase from {initial_failure_pct:.1}% to {failure_pct:.1}%"
    );
