...
pub trait DeferStore: Clone + Send + Sync + 'static {
    ...
    fn defer_additional_message(
        ...
        async move {
            self.append_deferred_message(key_id, offset, expected_retry_time)
                .await
        }
    }
    ...
    fn increment_retry_count(
        ...
        async move {
            ...
            self.set_retry_count(key_id, new_count).await?;
            Ok(new_count)
        }
    }
