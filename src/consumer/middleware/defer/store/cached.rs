    ...
    S: DeferStore,
{
    ...
    async fn complete_retry_success(
        ...
        // Write through to store first
        let result = self.store.complete_retry_success(defer_key, offset).await?;
    ...
    async fn get_next_deferred_message(
        ...
        let result = self.store.get_next_deferred_message(defer_key).await?;
    ...
    async fn set_retry_count(&self, defer_key: &Uuid, retry_count: u32) -> Result<(), Self::Error> {
        // Write through to store first
        self.store.set_retry_count(defer_key, retry_count).await?;
    ...
    async fn delete_key(&self, defer_key: &Uuid) -> Result<(), Self::Error> {
        // Write through to store first
        self.store.delete_key(defer_key).await?;
...
mod tests {
    ...
    async fn test_cache_hit_on_repeated_get() -> color_eyre::Result<()> {
        ...
        let result1 = cached_store.get_next_deferred_message(&defer_key).await?;
        assert_eq!(result1, Some((offset, 0)));

        // Second get (cache hit - should return same result without querying store)
        let result2 = cached_store.get_next_deferred_message(&defer_key).await?;
    ...
    async fn test_cache_update_on_increment() -> color_eyre::Result<()> {
        ...
        cached_store
            .defer_first_message(&defer_key, offset, retry_time)
            .await?;
        cached_store.get_next_deferred_message(&defer_key).await?;

        // Increment retry count (should update cache in-place)
        let new_count = cached_store.increment_retry_count(&defer_key, 0).await?;
        assert_eq!(new_count, 1);

        // Next get should see updated retry_count (cache hit, no store query)
        let result = cached_store.get_next_deferred_message(&defer_key).await?;
        assert_eq!(result, Some((offset, 1)));

        // Increment again to verify it keeps working
        let new_count = cached_store.increment_retry_count(&defer_key, 1).await?;
    ...
    async fn test_cache_update_on_complete_success() -> color_eyre::Result<()> {
        ...
        cached_store
            ...
            .await?;
        cached_store
            .defer_additional_message(&defer_key, Offset::from(200_i64), retry_time)
            .await?;

        // Complete first message
        let result = cached_store
            .complete_retry_success(&defer_key, Offset::from(100_i64))
            .await?;
        ...
        let cached = cached_store.get_next_deferred_message(&defer_key).await?;
    ...
    async fn test_defer_additional_out_of_order() -> color_eyre::Result<()> {
        ...
        cached_store
            .defer_additional_message(&defer_key, Offset::from(50_i64), retry_time)
            .await?;

        // Next read should see offset 50 (the smaller one) as next (cache hit!)
        let after = cached_store.get_next_deferred_message(&defer_key).await?;
    ...
    async fn test_defer_additional_monotonic() -> color_eyre::Result<()> {
        ...
        cached_store
            .defer_additional_message(&defer_key, Offset::from(300_i64), retry_time)
            .await?;
    ...
    async fn test_delete_key_updates_cache() -> color_eyre::Result<()> {
        ...
        cached_store.delete_key(&defer_key).await?;
