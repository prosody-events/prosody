...
impl<C> Clone for DequeDescriptor<C> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<C> Copy for DequeDescriptor<C> {}

impl<C> fmt::Debug for DequeDescriptor<C> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DequeDescriptor")
...
impl<S: Clone, C> Clone for DequeHandle<S, C> {
    fn clone(&self) -> Self {
        Self {
