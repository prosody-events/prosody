...
/// collection; `None` binds 0 for indefinite retention. An over-ceiling
/// `Some(d)` (Cassandra rejects `USING TTL ?` values above `630_720_000`
/// seconds) is rejected at `CollectionDefRegistry::register` time — never
/// silently collapsed to `None`, which would turn a finite retention into
/// permanent storage.
///
