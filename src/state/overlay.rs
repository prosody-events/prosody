...
impl<L> Overlay<L> {
    ...
    /// buffer and `own` as the running handler's event.
    #[must_use]
    pub fn new(dirty: Arc<DirtyStore>, lower: L, own: EventRef) -> Self {
        Self { dirty, lower, own }
    }
    ...
    /// The running handler's event.
    #[must_use]
    pub fn own(&self) -> EventRef {
        self.own
    }
