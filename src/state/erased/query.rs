...
pub struct ErasedReadSource<Q, Item>(Arc<dyn Fn(Q) -> StateCursor<Item> + Send + Sync>);
...
pub(crate) fn read<Q: Default, Item>(
    source: impl Fn(Q) -> StateCursor<Item> + Send + Sync + 'static,
