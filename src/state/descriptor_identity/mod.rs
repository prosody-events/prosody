...
pub trait DescriptorIdentityStore: Send + Sync + 'static {
    ...
    fn register_identity<'a, 'b, 'c>(
        &'a self,
        group_id: &'b str,
        row: &'c DurableDescriptorIdentity,
    ) -> impl Future<Output = Result<RegisterOutcome, Self::Error>> + Send + use<'a, 'b, 'c, Self>;
}
