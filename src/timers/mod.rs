...
//!
//! This module provides a comprehensive timer system that integrates with Prosody's
//! event processing capabilities. It allows applications to schedule timer events
//! that fire at specific times and are processed alongside regular Kafka messages.
//!
//! ## Overview
//!
//! The timer system consists of several key components:
//!
//! - **Triggers**: Represent scheduled timer events with a key and execution time
...
//! - **Slabs**: Time-based partitioning for efficient timer storage and retrieval
//!
//! ## Key Features
//!
//! - **Persistent Storage**: Timers are stored persistently and survive application restarts
//! - **Distributed Operation**: Multiple consumer instances can coordinate timer processing
//! - **Key-based Organization**: Timers are organized by keys, maintaining ordering guarantees
//! - **Efficient Querying**: Time-based slab organization enables efficient range queries
//! - **Integration**: Seamless integration with existing event processing pipelines
//!
//! ## Usage
//!
...
//!
//! 1. **Storage Layer**: Persistent storage via [`TriggerStore`] implementations
...
//! - **Slab-based Partitioning**: Timers are organized into time-based slabs for efficient access
...
/// Represents a scheduled timer event with an associated key and execution time.
///
...
/// ## Key Properties
///
/// - **Key**: Associates the timer with a logical entity (e.g., user ID, order ID)
/// - **Time**: Specifies when the timer should execute using [`CompactDateTime`]
/// - **Span**: Provides distributed tracing context for the timer event
///
/// ## Ordering and Equality
///
/// Triggers are ordered first by key, then by time. The tracing span is excluded
/// from equality and hash calculations to ensure consistent behavior across
...
pub struct Trigger {
    /// The key that identifies the logical entity this timer is associated with.
    ///
    ...
    ///
    /// Represented as a [`CompactDateTime`] for efficient storage and comparison.
