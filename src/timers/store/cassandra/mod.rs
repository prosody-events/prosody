...
//! constructors, and the shared low-level helpers (`execute_unpaged_discard`,
//! for example) that all submodules call. The bulk of the logic lives in
//! focused submodules to keep individual files navigable.
//!
...
///
/// Each store serves one partition. Its clones share the timer-state cache.
/// [`CassandraTriggerStoreProvider`] creates the store.
#[derive(Clone, Educe)]
#[educe(Debug)]
...
impl CassandraTriggerStore {
    ...
    pub(super) fn calculate_ttl(&self, time: CompactDateTime) -> i32 {
