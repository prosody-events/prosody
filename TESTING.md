...
| Area | Home |
| --- | --- |
| Crate-wide (`integration_test_count`, `test_cassandra_config`; span capture: `captured_spans{,_filtered}`, `GlobalSpans`, `assert_span_relation`, `named`, `sampled_remote_context`; metric capture: `GlobalMetrics`, `label`, `assert_distinct_labels`) | `src/tests/test_util.rs` |
